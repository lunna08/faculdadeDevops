...
class TestQuiz(unittest.TestCase):
    ...
    @patch("builtins.input", side_effect=["2", "2", "1"])  # todas corretas
    ...
    @patch("builtins.input", side_effect=["1", "1", "1"])  # só a última correta
    @patch("sys.stdout", new_callable=StringIO)
    ...
    @patch("builtins.input", side_effect=["3", "1", "3"])  # todas erradas
    @patch("sys.stdout", new_callable=StringIO)
